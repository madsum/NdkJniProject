/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class com_ma_jni_TestJNIReference */

#ifndef _Included_com_ma_jni_TestJNIReference
#define _Included_com_ma_jni_TestJNIReference
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     com_ma_jni_TestJNIReference
 * Method:    getIntegerObject
 * Signature: (I)Ljava/lang/Integer;
 */
JNIEXPORT jobject JNICALL Java_com_ma_jni_TestJNIReference_getIntegerObject
  (JNIEnv *, jobject, jint);

/*
 * Class:     com_ma_jni_TestJNIReference
 * Method:    anotherGetIntegerObject
 * Signature: (I)Ljava/lang/Integer;
 */
JNIEXPORT jobject JNICALL Java_com_ma_jni_TestJNIReference_anotherGetIntegerObject
  (JNIEnv *, jobject, jint);

#ifdef __cplusplus
}
#endif
#endif
